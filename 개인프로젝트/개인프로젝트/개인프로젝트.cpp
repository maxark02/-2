...
bool checkln(int x, int y, int cx, int cy, int r) {
...
void mouseCallback(ObjectID object, int x, int y, MouseAction action) {
	...
	if (object == problem) {
		...
		for (int i = 0; i < 7; i++) {
			...
			if ((checked[i] == false) && (checkln(x, y, leftX[i], Y[i], radius[i]) || checkln(x, y, rightX[i], Y[i], radius[i]))) {
				showObject(left[i]);
					showObject(right[i]);
...
int main() {
	...
	scene1 = createScene("problem", "problem.png");
